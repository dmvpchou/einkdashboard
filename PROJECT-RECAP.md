...
Leaf2 突然連不到頁面時，即使 Windows 已辨識 USB，也要確認 `adb devices` 是否為 `device`，並檢查 `adb reverse --list` 是否仍有 `tcp:8765`。裝置重開、USB 重接或 ADB daemon 重啟都會讓 reverse 消失；可用 `npm run install:autostart` 讓 watcher 自動補回。頁面仍是舊版時先重新載入，必要時暫時加上 `?v=N` 排除快取。

### 背景 server 與手動啟動會搶同一個 port

如果背景 watcher 已啟動 dashboard，再執行 `npm start` 會收到 `EADDRINUSE: address already in use 0.0.0.0:8765`。這不是 server 壞掉，而是 `8765` 已有正常服務；先用瀏覽器測試 `http://127.0.0.1:8765`。安裝自動啟動後應讓排程管理 server，不要再手動執行 `npm start`。
